...
if (isset($_POST['register_btn'])) {
    ...
    if (preg_match("!image!", $_FILES['picture1']['type'])) {
        $checker ++;
    }
    if (preg_match("!image!", $_FILES['picture2']['type'])) {
        $checker ++;
    }
    if ($checker < 3){
        exit;
            ...
            if ($user) { // if user exists
                ...
                copy($_FILES['picture1']['tmp_name'], $picture_path1);
                copy($_FILES['picture2']['tmp_name'], $picture_path2);

                $sql = "INSERT INTO users (fname, lname, email, phone, state, age, ighandle, picture, picture1, picture2, regno)"
                    . "VALUES ('$fname', '$lname', '$email', '$phone', '$state', '$age', '$ighandle', '$picture_path', '$picture_path1', '$picture_path2', '$regno')";
                    if($query){
                    ...
                    <div class="bg-dark p-4 p-lg-8 rounded-lg">
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype="multipart/form-data" autocomplete="on">
                            ...
                            <div class="form-group mb-4">
                                <label for="picture">Upload Head Shot (Passport)</label>
                                <input type="file" name="picture" id="picture" class="custom-input-file" required="" accept="image/*" />
                            </div>
                            <div class="form-group mb-4">
                                <label for="picture1">Upload Barebody or Top less</label>
                                <input type="file" name="picture1" id="picture1" class="custom-input-file" required="" accept="image/*" />
                            </div>
                            <div class="form-group mb-4">
                                <label for="picture2">Upload Full Body Shot (Casual)</label>
                                <input type="file" name="picture2" id="picture2" class="custom-input-file" required="" accept="image/*" />
                            </div>
